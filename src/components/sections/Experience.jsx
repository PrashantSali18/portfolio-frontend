...
export default function Experience() {
  return (
    <section id="experience" className="relative">
      <Container>
        <SectionTitle number="03 / Experience" title="Work" highlight="History" />
        ...
        <div className="relative max-w-3xl">
          ...
          >
            {experience.map((job) => (
              <motion.div key={job.id} variants={fadeLeft} className="flex gap-8">
                {/* Timeline dot */}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          ...
                          <span className="font-body text-muted text-xs">{job.location}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                        {job.current && (
                          <span className="tag tag-green text-[10px]">Current</span>
                        )}
